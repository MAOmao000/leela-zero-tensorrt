...
- Minimize timeouts (e.g. even if the timeout is set to 1 second, it takes about 1.1 seconds).
- Control drain-resume to reduce look-ahead time without raising an exception.
- Collecting statistics for analysis in a separate thread.
- Incorporating KataGo's dynamic-variance-scaled-cpuct.
- Exclusive control of tuning files and plan cache files.
- Incorporate as many suggestions from clang-analyzer-cplusplus and Visual Studio c++ Linter as possible.
